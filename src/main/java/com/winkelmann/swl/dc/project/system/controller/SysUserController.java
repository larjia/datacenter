...
import com.winkelmann.swl.dc.project.system.service.ISysPositionService;
...
public class SysUserController extends BaseController
{
	...
	public AjaxResult getInfo(@PathVariable(value = "userId", required = false) Long userId)
	{
		...
		ajax.put("positions", positionService.selectPositionAll());
		if (StringUtils.isNotNull(userId))
		{
			ajax.put(AjaxResult.DATA_TAG, userService.selectUserById(userId));
			ajax.put("positionId", positionService.selectPositionListByUserId(userId));
			ajax.put("roleId", roleService.selectRoleListByUserId(userId));
		}
		return ajax;
	}
	...
	public AjaxResult add(@Validated @RequestBody SysUser user)
	{
		...
		{
			return AjaxResult.error("新增用户'" + user.getPhoneNumber() + "'失败,手机号码已存在.");
		}
